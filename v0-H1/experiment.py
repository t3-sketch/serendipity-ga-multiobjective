...
# Candidate generators under comparison. The backbone hyperparameters repeat the
# values RecBole's SASRec.yaml gave the v0 baseline, so an arm differs from that
# baseline only in its architecture and loss.
BACKBONE_PARAMS = {"n_layers": 2, "n_heads": 2, "hidden_size": 64, "inner_size": 256,
                   "hidden_dropout_prob": 0.5, "attn_dropout_prob": 0.5,
                   "hidden_act": "gelu", "layer_norm_eps": 1e-12,
                   "initializer_range": 0.02, "loss_type": "CE"}


def model_registry(name):
    """Return the model class and its RecBole parameters for one candidate generator."""
    if name == "sasrec":
        from recbole.model.sequential_recommender.sasrec import SASRec
        return SASRec, dict(BACKBONE_PARAMS)
    if name == "comirec":
        from models.comirec import ComiRecSA
        return ComiRecSA, {**BACKBONE_PARAMS, "n_interests": 4, "attn_hidden_size": 256}
    if name == "esasrec":
        from models.esasrec import ESASRec
        return ESASRec, {**BACKBONE_PARAMS, "loss_type": "SS", "ff_emb_mult": 4,
                         "n_negatives": 256}
    raise ValueError(f"Unknown model {name}; choose sasrec, comirec or esasrec")


def build_model(config, cfg, dataset):
    model_class, _ = model_registry(config["model"])
    return model_class(cfg, dataset)
...
def fit_model(cfg, dataset, loader, valid_cases, genre_frame, config, output):
    ...
    history = []
    model = build_model(config, cfg, dataset)
...
def reuse_model(source, cfg, dataset, config, output):
    ...
    if prior.get("model", "sasrec") != config["model"]:
        raise ValueError("Checkpoint was trained for a different candidate generator")
...
def main():
    ...
    parser.add_argument("--model", choices=["sasrec", "comirec", "esasrec"],
                        help="Candidate generator; everything after it stays fixed")
    ...
    config.setdefault("model", "sasrec")
    if args.model is not None:
        config["model"] = args.model
    model_registry(config["model"])
