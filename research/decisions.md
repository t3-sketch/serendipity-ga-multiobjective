...
## 2026-09-12：就活向けKG比較と後続の人間評価研究を二段階で計画する

**課題**：就活までに完結する成果と、LLM評価器・人間のserendipityとのずれを検証する長期研究を区切る必要がある。

**判断（ユーザー採用の大枠）**：就活までにKGの比較研究をまとめ、Graph-Recを成果のデモとする。その限界を出発点にLLM・人間評価の研究計画を作る。

**仮説・根拠**：固定候補での距離・属性・関係構造の比較は、人間評価の完了を待たずにsystem-sideの研究として区切れる。後続研究では同じ候補に本人回答を対応付け、proxyとLLMの妥当性を検証する。実環境の利用者獲得を小規模参加者実験の前提にしない。

**結果**：依頼に基づきNotionに二つの計画草案を作成した。具体的な対象領域、比較式、人数、期限、Graph-Rec仕様は提案または未確認であり、実験条件としての採用ではない。新規実験・デモ実装・公開・募集は未実施。

**未完**：就活期限、Graph-Rec対象領域、1M保存物の再利用可否、データ源、比較条件の確定。v0の保持とM1の既存未完了事項、M3のhuman validation条件は維持する。

## 2026-09-13：1M公開入口をreproduceへ移し、H1を再現証拠として記録する

**課題**：公開コードは100K用のままで、1M正式結果へ到達する経路が個人の絶対パスに依存していた。

**判断**：別保存の1M成果物は移動せず、公開snapshotを`v0/reproduce/`に置く。100Kの`v0/experiment.py`は歴史的snapshotとして残し、現行入口から外す。H1はv0を置き換えず、再現と候補生成器比較の証拠として[S02](../v0/reports/S02-h1-candidate-generators.md)へ要約する。

**根拠**：H1 A0/A1が保存指標を差0.0で再現した。公開対象だけで`check.py`と集計照合ができる。フル再学習は費用の別単位とする。

**結果**：S01報告値と公開expected CSVを再照合した。公開`experiment.py`はresumed2記録hashともH1 harnessともバイト一致しない。sanity checkは別protocolの途中記録であり、test未評価。

**未完**：事前仕様との照合、同一ソースhashの復元、フル再学習、H1/sanityフォルダのcommit選別。

## 2026-09-13：Phase 3はmock bundle 0.1のオフライン接続検証に限定する

**課題**：製品側の推薦出力と研究側の検査を二重管理すると、何を評価したか追えなくなる。

**判断**：共通SDKは作らず、Engineeringの `docs/research-export.md` version 0.1に従い、1推薦要求を1 caseとしてmock出力だけを固定bundleで渡す。本人評定とLLM予測は空ファイルのまま残し、尺度はPhase 5で決める。
